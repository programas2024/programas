...
class RealizarDeposito:
    def __init__(self, master):
        self.master = master
        self.master.title("Realizar Depósito")
        self.master.geometry("400x200")
        self.master.configure(bg="white")

        self.label_titulo = tk.Label(master, text="Realizar Depósito", font=("Helvetica", 16), bg="white")
        ...
        self.btn_deposito = tk.Button(master, text="Realizar Depósito", command=self.realizar_deposito, bg="white", state=tk.DISABLED)
        self.btn_deposito.place(x=120, y=150)
        ...
        # Verificar y crear la tabla Deposito si no existe
        self.verificar_tabla_deposito()
    ...
    def verificar_tabla_deposito(self):
        try:
            ...
            cursor.execute("CREATE TABLE IF NOT EXISTS Deposito (id INT AUTO_INCREMENT PRIMARY KEY, numero_cuenta VARCHAR(255), monto DECIMAL(10, 2), numero_transaccion VARCHAR(255), fecha_hora DATETIME)")
            ...
            print("Error al verificar la tabla Deposito:", error)
        ...
        if numero_cuenta:
            try:
                ...
                if cuenta:
                    ...
                    self.btn_deposito.config(state=tk.NORMAL)
                    ...
                    messagebox.showinfo("Éxito", "Cuenta encontrada. Puede realizar el depósito.")
                else:
                    self.entry_monto.config(state=tk.DISABLED)
                    self.btn_deposito.config(state=tk.DISABLED)
    ...
    def realizar_deposito(self):
        ...
        try:
            ...
            # Insertar los datos del depósito
            query = "INSERT INTO Deposito (numero_cuenta, monto, numero_transaccion, fecha_hora) VALUES (%s, %s, %s, %s)"
            ...
            print("Error al realizar depósito:", error)
